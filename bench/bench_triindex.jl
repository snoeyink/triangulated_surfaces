...
function k_array_n!(tri::Array{UInt16,3}, r::Vector{Int})
    @inbounds for _ in 1:1000, i in 1:998
        tri[r[i], r[i+1], r[i+2]] += UInt16(1)
    end
end

function k_array_16!(tri::Array{UInt16,3}, r::Vector{Int})
    @inbounds for _ in 1:1000, i in 1:998
        tri[r[i], r[i+1], r[i+2]] += UInt16(1)
    end
end
...
function k_flat_n!(tri::Vector{UInt16}, r::Vector{Int})
    @inbounds for _ in 1:1000, i in 1:998
...
function k_flat_16!(tri::Vector{UInt16}, r::Vector{Int})
    @inbounds for _ in 1:1000, i in 1:998
...
arr_n    = zeros(UInt16, n, n, n)     # 2197 elements, 4.3 KB
arr_16   = zeros(UInt16, P, P, P)     # 4096 elements, 8.0 KB
flat_n   = zeros(UInt16, n^3)         # 2197 elements, 4.3 KB
flat_16  = zeros(UInt16, P^3)         # 4096 elements, 8.0 KB
...
# ─── Benchmarks ───────────────────────────────────────────────────────────────
println("=== 1. Array{UInt16,3} $(n)×$(n)×$(n)  (runtime stride=$(n)) ===")
display(@benchmark k_array_n!($arr_n, $r_bench))

println("\n=== 2. Array{UInt16,3} $(P)×$(P)×$(P)  (runtime stride=$(P)) ===")
display(@benchmark k_array_16!($arr_16, $r_bench))

println("\n=== 3. flat Vector{UInt16} length $(n^3), hardcoded stride=$n ===")
display(@benchmark k_flat_n!($flat_n, $r_bench))

println("\n=== 4. flat Vector{UInt16} length $(P^3), bit-shift index ===")
